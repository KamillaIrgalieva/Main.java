...
Реализация
Создайте массив и заполните его из примера
Напишите функции mark и build
Вызовите функцию build от массива, убедитесь, что вывод совпал с тем что в примере
(необязательно) Оцените алгоритмическую сложность алгоритма, сколько он занимает времени и памяти. Аргументируйте, почему вы так считаете.
...
public class Main {
    ...
    public static void mark ( int[] arr, int left, int right, int level, int[] levels){
        if (left == right) {
            ...
            levels[middle] = level;
            level++;
            mark(arr, left, middle - 1, level, levels);
            mark(arr, middle + 1, right, level, levels);
        }
    }
